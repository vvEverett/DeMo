...
from .expert_label_loader import ExpertLabelLoader
...
class Av2Dataset(Dataset):
    def __init__(
        ...
        # Load expert labels for supervised MoE training
        self.expert_label_loader = ExpertLabelLoader(classification_csv)
    ...
    def __getitem__(self, index: int):
        ...
        # Load preprocessed data (safe - files are generated by our own preprocessing)
        data = torch.load(file_path, weights_only=False)
        
        # Get expert label for this scenario
        filename = file_path.name
        expert_label = self.expert_label_loader.get_label(filename)
        
        data = self.process(data, expert_label)
        return data
    
    def process(self, data, expert_label=None):
        sequence_data = []
        ...
        for cur_step in self.sequence_origins:
            for ag_idx in train_idx:
                ag_dict = self.process_single_agent(data, ag_idx, cur_step, expert_label)
            sequence_data.append(ag_dict)
        
        return sequence_data

    def process_single_agent(self, data, idx, step=50, expert_label=None):
        ...
        return {
            ...
            'expert_label': expert_label,  # Add expert label (1-5 or None)
        }
...
def collate_fn(seq_batch):
    ...
    for i in range(len(seq_batch[0])):
        ...
        expert_labels = [b['expert_label'] for b in batch]
        if all(label is not None for label in expert_labels):
            data['expert_label'] = torch.tensor(expert_labels, dtype=torch.long)
        else:
            data['expert_label'] = None
