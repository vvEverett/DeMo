...
from .layers.time_decoder_gmm import TimeDecoder
...
# GMM Baseline - only 'DeMo' with GMM predictor (Mode Localization Module only)
class ModelForecast(nn.Module):
    def __init__(
        ...
        # Mode Localization Module with GMM predictor
        self.time_decoder = TimeDecoder(future_len=future_steps, dim=embed_dim)
    ...
    def load_from_checkpoint(self, ckpt_path):
        ...
        print('Pretrained weights have been loaded (Mode Localization Module with GMM only).')
        return self
    ...
    def forward(self, data):
        ...
        ###### Trajectory decoding with Mode Localization Module only (GMM-based) ###### 
        ...
        ret_dict = {
            "y_hat": y_hat,  # trajectory output from Mode Localization Module [B, K, T, 2]
            "pi": pi,  # probability output from Mode Localization Module [B, K]
            "scal": scal,  # scale parameter for Laplace loss [B, K, T, 2]
