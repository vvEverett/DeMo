...
from .layers.time_decoder_moe_supervised import TimeDecoder
...
class ModelForecast(nn.Module):
    ...
    def __init__(
        ...
        # MoE Time Decoder with supervised routing (per-mode) and Gumbel noise regularization
    ...
    def load_from_checkpoint(self, ckpt_path):
        """Load pretrained weights from DeMo baseline"""
        ...
        # Filter out parameters from removed modules and MoE-specific modules
        filtered_state_dict = {}
        removed_keys = []
        ...
        for k, v in state_dict.items():
            ...
            if any(x in k for x in ['self_block_dense', 'cross_block_dense', 'self_block_different_mode',
                ...
                removed_keys.append(k)
                continue
            # Skip MLP predictor parameters (will be replaced by MoE)
            if 'predictor' in k:
                removed_keys.append(k)
                continue
        ...
        print(f"Removed {len(removed_keys)} parameters from removed/replaced modules")
        
        missing_keys, unexpected_keys = self.load_state_dict(state_dict=filtered_state_dict, strict=False)
        
        if missing_keys:
            print(f"Missing keys (newly initialized): {len(missing_keys)} keys")
            print(f"  Including MoE components: {[k for k in missing_keys if 'predictor' in k or 'router' in k or 'expert' in k][:5]}")
        if unexpected_keys:
            print(f"Unexpected keys (ignored): {unexpected_keys}")
            
        print('Pretrained weights loaded. MoE predictor initialized randomly.')
        return self

    def forward(self, data):
        # Extract expert labels if available (for supervised training)
        expert_labels = data.get("expert_label", None)  # [B] - ground truth expert IDs (1-5)
        ...
        ###### Trajectory decoding with Supervised MoE ###### 
        ...
        ret_dict = {
            ...
            "router_logits": router_logits,  # router logits for supervision [B, M, num_unshared_experts]
